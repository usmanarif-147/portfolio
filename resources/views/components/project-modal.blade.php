...
<div x-data="{
    ...
    go(i) { if (this.count()) this.slide = (i + this.count()) % this.count() },
...
}" @open-project.window="show($event.detail)" @keydown.escape.window="if (open) close()"
    @keydown.arrow-right.window="if (open) next()" @keydown.arrow-left.window="if (open) prev()" x-cloak>
    <div x-show="open" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" style="display:none">
        {{-- Backdrop --}}
        <div x-show="open" x-transition.opacity.duration.200ms class="absolute inset-0 bg-black/80 backdrop-blur-sm"
            @click="close()"></div>
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="group relative w-full shrink-0 overflow-hidden bg-dark-950 h-[42vh] sm:h-[52vh] md:h-[58vh]">
                <template x-for="(img, i) in (project ? project.images : [])" :key="i">
                    <img :src="img" :alt="project.title" x-show="slide === i"
                ...
                <template x-if="project && !project.images.length">
                    <div class="flex h-full w-full items-center justify-center text-sm text-gray-600">No images for this
                        project.</div>
                </template>

                {{-- subtle top vignette so the close button stays legible --}}
                <div
                    class="pointer-events-none absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-black/40 to-transparent">
                </div>
                ...
                <template x-if="project && project.images.length > 1">
                    <div>
                        ...
                        {{-- ‹ n / total › pill --}}
                        <div
                            class="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-white/[0.08] bg-black/60 px-2 py-1 backdrop-blur-sm">
                            <button type="button" @click="prev()"
                                class="flex h-6 w-6 items-center justify-center rounded-full text-white/80 hover:bg-white/10 hover:text-white"
                                aria-label="Previous">
                            ...
                            <span class="min-w-[42px] text-center text-xs font-medium tabular-nums text-white"
                ...
                <div class="flex shrink-0 gap-2 overflow-x-auto border-t border-white/[0.04] bg-dark-950/40 px-4 py-3">
                    <template x-for="(img, i) in project.images" :key="i">
                        <button type="button" @click="go(i)"
                            ...
                            :class="slide === i ? 'border-transparent ring-2 ring-accent' :
                ...
                <template x-if="project && project.tech.length">
                    <div class="mb-7 flex flex-wrap gap-2">
                        <template x-for="t in project.tech" :key="t">
                            <span
                                class="rounded-full border border-accent/10 bg-accent/10 px-3 py-1.5 text-sm text-accent-light"
                                x-text="t"></span>
                        </template>
                    </div>
                </template>
