...
<div x-data="{}" role="button" tabindex="0" @click="$dispatch('open-project', @js($payload))"
    ...
    <div class="relative overflow-hidden">
        ...
        @else
            <div class="flex h-48 w-full items-center justify-center bg-dark-700">
                <span
                    class="text-4xl font-extrabold text-accent/20">{{ strtoupper(substr($project->title, 0, 1)) }}</span>
            </div>
        @endif
    ...
    <div class="flex flex-1 flex-col p-6">
        <h3 class="mb-2 text-xl font-bold text-white transition-colors group-hover:text-accent">{{ $project->title }}
        </h3>
        ...
        @if (!empty($project->tech_stack))
            <div class="flex flex-wrap gap-2">
                @foreach ($project->tech_stack as $tech)
                    <span
                        class="rounded-full bg-accent/10 px-2 py-1 text-xs text-accent-light">{{ $tech }}</span>
                @endforeach
            </div>
        @endif
    </div>
